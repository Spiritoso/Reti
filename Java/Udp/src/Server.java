...
public class Server {
	...
	public static void main(String[] args) {
		...
		try (DatagramSocket socket = new DatagramSocket(19900, add)){
			...
			DatagramPacket packet = new DatagramPacket(buf, buf.length, recadd, recport);		//anche senza recadd e recport va uguale
			//packet.setData(buf);		//aggancio il mio buffer
			socket.receive(packet);		//ricezione con attesa sincrona. La receive setta anche la 
			
			
			System.out.println("add: " + socket.getInetAddress() + " port: " + packet.getPort());
